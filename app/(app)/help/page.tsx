...
const TOC: { href: string; label: string }[] = [
  ...
  { href: "#dictionaries", label: "Довідники" },
    ...
    <main className="container mx-auto flex flex-1 flex-col gap-6 px-4 py-6 pb-24 md:pb-6">
      <header className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Розгорніть секцію, щоб прочитати деталі. Натисніть на пункт у списку
          нижче для швидкого переходу.
        </p>
      </header>
      ...
      <section id="overview" className="scroll-mt-20">
        ...
        <Collapse summary="🌿 Що це за додаток?" defaultOpen>
          <p>
            <strong>ЛавандаОблік</strong> — облікова PWA для невеликого бізнесу
            (фермерські солодощі, варення, продукція з лавандою). Веде
            замовлення, рух коштів і базові аналітичні зрізи. Дані
            синхронізуються в реальному часі між пристроями всіх членів команди.
          </p>
          <FeatureGrid
            items={[
              {
                title: "📋 Замовлення",
                desc:
                  "Lifecycle Нове → Підтверджено → В роботі → Готове з фото, доставкою і коментарями",
              },
              {
                title: "💸 Транзакції",
                desc:
      ...
      <section id="orders" className="scroll-mt-20">
        ...
        <Collapse summary="📋 Створення та редагування">
          <p>
          ...
          <p>
            Кнопка <strong>«+ Замовлення»</strong> (десктоп) або плаваюча кнопка{" "}
            <strong>«+»</strong> внизу праворуч (мобільний).
          </p>
          <Step n={1}>
          ...
          <Step n={3}>
            ...
            позицію». Якщо товару немає в довіднику — створіть інлайн.
          </Step>
          <Step n={4}>
      ...
      <section id="photos-voice" className="scroll-mt-20">
        <Collapse summary="📷 Фото · 🎙 Голосовий ввід · 💬 Коментарі">
          <H3>Фото</H3>
          <p>
      ...
      <section id="transactions" className="scroll-mt-20">
        ...
        <Collapse summary="💸 Транзакції">
          ...
          <Step n={5}>
            <strong>Ціна</strong>, <strong>кількість</strong>, <strong>дата</strong>{" "}
          ...
          <p>
            При переведенні замовлення в статус <strong>«Готове»</strong>{" "}
            автоматично створюються транзакції доходу — по одній на кожну
      ...
      <section id="dictionaries" className="scroll-mt-20">
        <SectionGroup>Довідники</SectionGroup>

        <Collapse summary="🏷 Категорії">
          <p>
            <code>/settings/categories/</code> — джерело категорій для
            замовлень і транзакцій. Кожна має <strong>тип</strong> (income або
            expense) — він визначає, де категорія буде доступна.
          </p>
          <ul className="my-2 ml-5 list-disc space-y-1 text-sm text-muted-foreground">
            <li>У формі замовлення доступні лише <strong>income</strong>.</li>
            <li>
              У формі транзакції — залежно від обраного типу (дохід / витрата).
            </li>
            <li>Колір категорії використовується у бейджах і графіках.</li>
          </ul>
        </Collapse>

        <Collapse summary="📦 Товари">
          <p>
            <code>/settings/products/</code> — прайс-лист. Поля: назва, одиниця
            (шт / кг / уп.), ціна за замовчуванням, категорія за замовчуванням.
          </p>
          <p className="text-sm text-muted-foreground">
            Коли товар обрано у формі замовлення / транзакції — ціна і
            категорія підтягуються автоматично. Можна перевизначити.
          </p>
        </Collapse>

        <Collapse summary="🚛 Постачальники">
          <p>
            <code>/settings/suppliers/</code> — магазини, фермери, агровіни,
            у кого закуповуємо сировину. Використовується у експенс-транзакціях
            для фільтрації звітів.
          </p>
        </Collapse>

        <Collapse summary="👤 Клієнти">
          <p>
            <code>/settings/customers/</code> — покупці. Поля: ім'я, телефон,
            адреса, вік, джерело (звідки прийшов), нотатки.
          </p>
          <p className="text-sm text-muted-foreground">
            При створенні замовлення для нового клієнта — телефон/адреса
            автоматично записуються у його картку (м'який backfill: не
            перезаписує, якщо вже задано).
          </p>
        </Collapse>
      </section>
      ...
      <section id="import" className="scroll-mt-20">
        ...
        <Collapse summary="📥 Імпорт з Excel">
          <p>
            <code>/settings/import/</code> — імпорт історії транзакцій з .xlsx.
            Корисно для першого завантаження старих даних з електронної
            таблиці.
          </p>
          <p className="text-sm text-muted-foreground">
        ...
        <Collapse summary="🔔 Telegram-сповіщення">
          <p>
      ...
      <section id="users" className="scroll-mt-20">
        ...
        <Collapse summary="👥 Користувачі та ролі">
      ...
      <section id="data" className="scroll-mt-20">
        <Collapse summary="🔐 Дані та безпека">
          <p>
...
function Collapse({
  ...
  return (
    <details
      open={defaultOpen}
      className="group mb-3 rounded-lg border bg-card shadow-sm transition-shadow open:shadow-md"
    >
